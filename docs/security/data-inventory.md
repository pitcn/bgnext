...
On every client, `currencies` and `items` may contain only values whose keys are present in that client family's explicit resource-column whitelist. An available API alone does not authorize collection. The current permanent-60, TBC, Mists and Retail catalogs declare no currency, special-equipment or upgrade-item keys, so those values are neither read nor stored.
