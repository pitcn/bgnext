...
do
    ...
    if BG.IsTBC then
        -- TBC 端只注册 BT 帧（Maxb=16 需 14 boss 槽），按 v2.3.5 合并海山5+黑庙9
        for _, v in ipairs(boss2) do
            tinsert(boss, v)
        end
        Addother(boss)
        AddDB("BT", boss)
    else
        -- WLK/vanilla 保留拆分：HS/BT 各自独立帧（修复 v2.3.5 合并表在 WLK 的错位显示/HS nil）
        Addother(boss)
        AddDB("HS", boss)
        Addother(boss2)
        AddDB("BT", boss2)
