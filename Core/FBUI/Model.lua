...
function BG.CreateBossModel()
    ...
    if BG.IsTitan then
        ...
        model:SetPitch(0.3)
        model:SetRoll(0)
        local model = CreateBossModel("TOCtitan", 10, 0, 80, 14834, 1)
        model:SetPosition(13, -2.5, -1)
        model:SetFacing(-0.5)
        model:SetPitch(0.1)
        model:SetRoll(0)
        ...
        model:SetPitch(0)
        model:SetRoll(0)
        local model = CreateBossModel("SWtitan", 6, -27, -65, 23863, .7)
        model:SetPosition(-1, 0, 0)
        model:SetFacing(0)
        model:SetPitch(0)
        model:SetRoll(0)
        -- CreateAllTestButton(model)
        local model = CreateBossModel("SWtitan", 13, 50, 10, 25315, .75)
        model:SetPosition(0, 0, 1)
        model:SetFacing(-.1)
        model:SetPitch(0.53)
        model:SetRoll(0)
        local modelFrame = CreateFrame("PlayerModel", nil, model, "BackdropTemplate")
        modelFrame:SetSize(400, 400)
        modelFrame:SetPoint("CENTER", -20, -30)
        modelFrame:SetModel(194045) -- 166112,166149,166303,167000火焰 194045火墙
        modelFrame:SetPosition(-40, -6, 0)
        modelFrame:SetFrameLevel(model:GetFrameLevel() + 1)
    ...
    if BG.IsMOP then
        ...
        model:SetRoll(0)            -- 倾斜
        -- CreateAllTestButton(model)
        local model = CreateBossModel("SOO", 14, 5, -20, 71865, .55)
        model:SetPosition(-0, 0, 0) -- Z,X,Y
        model:SetFacing(0)          -- 左右
        model:SetPitch(0)           -- 上下
        model:SetRoll(0)            -- 倾斜
