...
function Runtime:build(raidId)
    ...
    self.options.pairItems(raidId, function(_, buyer, money, bossIndex, rowIndex)
        ...
        local normalizeName = self.options.normalizeName
        local key = type(normalizeName) == "function" and normalizeName(name) or name
        if type(key) == "string" and key ~= "" then
            fines[key] = fines[key] or 0
            debts[key] = debts[key] or 0
            if bossIndex == maxBoss then
                fines[key] = fines[key] + (tonumber(money:GetText()) or 0)
            end
            debts[key] = debts[key]
