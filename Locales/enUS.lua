...
do
    ...
    L["普通团员启用时会停用交易记账以避免冲突；团长或物品分配者仍保留交易记账。"] = "For ordinary raid members this disables trade bookkeeping to avoid duplicate entries; raid leaders and master looters keep trade bookkeeping enabled."
    ...
    L["合并导入"] = "Merge"
    L["替换导入"] = "Replace"
    L["心愿已发生变化，请重新预览后再导入。"] = "Your wishlist changed. Preview the import again before applying it."
    L["心愿导入失败：当前角色信息不可用。"] = "Wishlist import failed: the current character is unavailable."
    L["心愿清单导入成功：新增%d，覆盖%d，删除%d。"] = "Wishlist imported: %d added, %d overwritten, %d deleted."
    L["心愿导入失败：%s"] = "Wishlist import failed: %s"
    L["确认合并心愿？涉及%d个副本：新增%d，覆盖%d，删除%d。"] = "Merge wishlist data for %d raid(s)? %d added, %d overwritten, %d deleted."
    L["确认替换心愿？涉及%d个副本：新增%d，覆盖%d，删除%d。"] = "Replace wishlist data for %d raid(s)? %d added, %d overwritten, %d deleted."
    ...
    L["插件：%s"] = "AddOn: %s"
    L["团队拍卖：已就绪 %s"] = "Raid auction: ready %s"
    L["团队拍卖：未组团"] = "Raid auction: not in a raid"
    L["团队拍卖就绪检查"] = "Raid auction readiness check"
    L["进入团队后可检查团员的拍卖兼容状态。"] = "Join a raid to check each member's auction compatibility."
    L["左键：重新检测（仅团长或拾取负责人）"] = "Left-click: check again (raid leader or master looter only)"
    L["未响应不代表未安装，可让团员重载界面后重新检测。"] = "No response does not prove the addon is missing. Ask the member to reload and check again."
    L["已就绪"] = "Ready"
    L["插件已响应，拍卖端未响应"] = "Addon responded; auction endpoint did not"
    L["未响应"] = "No response"
    L["离线"] = "Offline"
    L["拍卖WA版本"] = "Auction WA version"
    ...
    L["拍卖"] = "Auction"
    L["拍卖当前掉落"] = "Auction current loot"
    L["把当前可拍物品加入待拍队列，仍需逐件确认。"] = "Add eligible current loot to the pending auction queue. Each item still requires confirmation."
    L["没有可拍卖的物品"] = "No eligible loot to auction"
    L["仅团长或拾取负责人可以发起拍卖"] = "Only the raid leader or master looter can start auctions"
    L["仅在副本拾取时提供一键拍卖"] = "One-click auction is available for instance loot only"
    ...
    L["不过滤装备"] = "Do not filter gear"
    L["显示全部装备；再次点击或选择其他方案可恢复过滤"] = "Show all gear; click again or choose another scheme to restore filtering"
...
do -- BGNext auction price presets
    L["价格预设"] = "Price Presets"
    L["团长起拍价"] = "Leader Starting Price"
    L["我的心理价"] = "My Price Limit"
    L["用于团长开团时自动填入起拍价；单件价优先，修改基础价只影响未单独设置的装备。"] = "Prefills the leader's starting price. Per-item prices take priority; changing the base affects only items without an override."
    L["用于参团时自动填入已保存的心理价，不会自动启用或发送。"] = "Automatically fills your saved price limit when joining; it is never enabled or sent automatically."
    L["基础"] = "Base"
    L["基础价"] = "Base price"
    L["单件"] = "Item"
    L["未设置"] = "Not set"
    ...
    L["已设置"] = "Set"
    L["件装备"] = "items"
    L["清除本团本心理价"] = "Clear this raid's price limits"
    L["全部"] = "All"
    L["清除筛选"] = "Clear filters"
    L["全部装备"] = "All loot"
    L["杂项"] = "Misc"
    L["新方案名称"] = "New preset name"
    L["无法创建方案。"] = "Could not create preset."
    L["无法复制方案。"] = "Could not copy preset."
    L["名称无效。"] = "Invalid name."
    L["至少保留一套方案。"] = "Keep at least one preset."
    L["确定删除当前方案？"] = "Delete the current preset?"
    ...
    L["天神首胜"] = "Celestial first win"
    L["农场收菜"] = "Farm harvest"
    L["火牛"] = "Ordos"
    L["野牛人火神斡耳朵斯"] = "Ordos, Fire-God of the Yaungol"
    L["未解锁"] = "Locked"
    ...
    L["清空队列"] = "Clear queue"
    L["确定清空待拍队列吗？\n关闭窗口不会清空队列。"] = "Clear the pending auction queue?\nClosing this window does not clear the queue."
    L["暂无待拍装备\n可粘贴装备链接，或从拾取窗口加入。"] = "No pending items\nPaste an item link or add one from the loot window."
    L["点击装备确认价格并开拍"] = "Click an item to confirm its price and start the auction"
    L["单件价"] = "Per-item"
    L["手动输入"] = "Manual"
    L["移除"] = "Remove"
    L["上移"] = "Up"
...
do -- BGNext feature management and guide catalog
    L["功能管理"] = "Feature Management"
    L["BGNext 功能管理"] = "BGNext Feature Management"
    L["完整模式"] = "Full Mode"
    L["基础模式"] = "Basic Mode"
    L["自定义模式"] = "Custom Mode"
    L["当前模式："] = "Current mode: "
    L["（必需）"] = "(Required)"
    L["个人工具"] = "Personal Tools"
    L["拍卖工具"] = "Auction Tools"
    L["结算工具"] = "Settlement Tools"
    L["界面与隐私"] = "Interface & Privacy"
    L["心愿清单"] = "Wishlist"
    L["记录装备需求，并用备选、次 BIS、BIS 表示优先级。"] = "Track gear needs with Backup, 2nd BiS and BiS priorities."
    L["角色总览"] = "Character Overview"
    L["集中查看自己登录过的角色信息和进度。"] = "Review information and progress for characters you have logged into."
    L["装备过滤"] = "Equipment Filter"
    L["按角色与专精减少不相关装备显示。"] = "Reduce irrelevant gear rows based on character and specialization."
    L["基础兼容"] = "Core Compatibility"
    L["保持与官方 BGLite 的表格和团队流程兼容。"] = "Keep bill and raid workflows compatible with official BGLite."
    L["开拍安全检查"] = "Auction Safety Checks"
    L["发送拍卖前复核权限、团队、战斗状态和价格。"] = "Recheck permissions, raid, combat state and price before sending an auction."
    L["价格预设"] = "Price Presets"
    L["保存团本起拍价方案和单件价格。"] = "Save raid starting-price schemes and per-item prices."
    L["待拍队列"] = "Auction Queue"
    L["把多件装备加入队列，逐件确认后开拍。"] = "Queue several items and confirm each one before starting its auction."
    L["交易事实采集"] = "Trade Capture"
    L["在本地记录当前团的实际交易结果。"] = "Record actual trade results for the current raid locally."
    L["数据生命周期"] = "Data Lifecycle"
    L["限制当前结算数据的范围和保留时间。"] = "Limit the scope and retention of current-settlement data."
    L["结算记录与检查"] = "Settlement Records & Checks"
...
do -- BGNext in-game guide
    ...
    L["只清当前副本"] = "Clear current raid only"
    L["清理所有%s副本"] = "Clear all %s raids"
    L["检测到新副本进度，表格< %s >的当前副本区间（Boss %s-%s）仍有内容。只清当前副本时，其他副本、杂项、罚款、支出和当前团结算记录会保留；如需开始整个新阶段，请选择“清理所有%s副本”。"] = "A new lockout was detected, but table < %s > still has content in this raid range (Boss %s-%s). If only this raid is cleared, the other raid, shared miscellaneous rows, penalties, expenses, and the current settlement are kept. To start the whole phase fresh, choose Clear all %s raids."
    L["已清理所有%s副本，并清除共享的杂项、罚款和当前团结算记录；支出按现有清表设置处理。"] = "Cleared all %s raids, including shared miscellaneous rows, penalties, and the current settlement. Expenses follow the existing clear-table settings."
end
