...
do -- BGNext 旧历史数据清理
    L["存储与隐私"] = true
    L["旧历史数据清理"] = true
    L["BGNext 不会读取或迁移旧版历史表格、交易记录和邮件记录。若旧数据仍保存在本地，可在这里手动清理以减少常驻内存。"] = true
    L["清理 BGNext 中的旧历史数据"] = true
    L["此操作不可恢复。建议先退出游戏并备份 WTF 文件夹；当前账单、设置、心愿清单、当前团结算和角色总览不会被删除。"] = true
    L["检测到 %d 项旧历史数据。"] = true
    L["未检测到旧历史数据。"] = true
    L["确定清理 BGNext 中的旧历史数据吗？\n\n此操作不可恢复。建议先退出游戏并备份 WTF 文件夹。只会删除旧版历史表格、交易记录和邮件记录。"] = true
    L["确认清理"] = true
end
