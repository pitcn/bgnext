return function(test)
    ...
    test.eq(base("cata", "WARRIOR").armor[2], true, "cata applies modern armor specialization")
    test.eq(base("cata", "WARRIOR").armor[4], nil, "cata keeps plate")
    ...
    test.eq(affix("tbc").ARMOR_PEN ~= nil, true, "tbc shows armor penetration")
    test.eq(affix("tbc").RESILIENCE ~= nil, true, "tbc shows resilience")
    test.eq(affix("titan").ARMOR_PEN ~= nil, true, "titan shows armor penetration")
    test.eq(affix("titan").RESILIENCE ~= nil, true, "titan shows resilience")
    test.eq(affix("mop").RESILIENCE ~= nil, true, "mop shows resilience")
    test.eq(affix("mop").DEFENSE, nil, "mop hides removed defense rating")
    test.eq(affix("mop").ARMOR_PEN, nil, "mop hides armor penetration")
    test.eq(affix("wrath").MASTERY, nil, "wrath hides mastery")
    test.eq(affix("wrath").VERSATILITY, nil, "wrath hides versatility")
    test.eq(affix("wrath").RESILIENCE ~= nil, true, "wrath shows resilience")
    test.eq(affix("cata").DEFENSE, nil, "cata hides removed defense rating")
    test.eq(affix("cata").MASTERY ~= nil, true, "cata shows mastery")
    test.eq(affix("cata").VERSATILITY, nil, "cata hides versatility")
    test.eq(affix("cata").RESILIENCE ~= nil, true, "cata shows resilience")
    ...
    local casterTbc = spec("tbc", "MAGE", "tree:MAGE:1")
    test.eq(casterTbc.affix.ARMOR_PEN, true, "tbc caster filters physical armor penetration")
