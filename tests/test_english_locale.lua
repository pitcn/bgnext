...
return function(test)
    ...
    for _, localized in ipairs({ zhCN, zhTW, enUS, enGB, deDE }) do
        ...
        test.eq(localized.updateText_now[1], "BGNext 0.8.1", "current in-game update notes use the release version")
        ...
        test.eq(releaseInfo.changelog[1], localized.L["修复部分 BGNext 窗口关闭后仍占用键盘输入，导致移动或聊天按键暂时失效的问题。"],
