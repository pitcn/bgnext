return function(test)
    ...
    test.eq(info.version, "0.2.3", "BGNext version is independent")
    ...
    test.eq(changelogItems:find("正式服", 1, true) ~= nil, true,
        "in-game changelog includes the retail fixes")
    test.eq(changelogItems:find("缩放", 1, true) ~= nil, true,
        "in-game changelog includes responsive scaling")
    test.eq(changelogItems:find("/biaoge", 1, true) ~= nil, true,
        "in-game changelog includes the compatibility command")
