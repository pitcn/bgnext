return function(test)
    ...
    test.eq(info.version, "0.8.4", "BGNext version is independent")
    ...
    for _, text in ipairs({ "买家", "金额", "欠款", "快捷键", "装备过滤", "对账", "拾取职责" }) do
    ...
    local expectedHistory = { "0.8.3", "0.8.2", "0.8.1", "0.8.0", "0.7.0", "0.6.0", "0.5.0", "0.4.0", "0.3.1", "0.3.0", "0.2.3", "0.2.2", "0.2.1", "0.2.0", "0.1.0" }
    ...
    test.eq(info.history[11].changelog[1]:find("正式服心愿", 1, true) ~= nil, true,
        "0.2.3 history describes its Retail display fixes")
    test.eq(info.history[12].changelog[1]:find("跨客户端拍卖编号", 1, true) ~= nil, true,
