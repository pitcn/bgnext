return function(test)
    ...
    test.eq(info.version, "0.8.1", "BGNext version is independent")
    ...
    for _, text in ipairs({ "键盘", "专业", "折叠", "买家", "成交金额", "团长工具", "跨服", "邮件" }) do
    ...
    local expectedHistory = { "0.8.0", "0.7.0", "0.6.0", "0.5.0", "0.4.0", "0.3.1", "0.3.0", "0.2.3", "0.2.2", "0.2.1", "0.2.0", "0.1.0" }
    ...
    test.eq(info.history[8].changelog[1]:find("正式服心愿", 1, true) ~= nil, true,
        "0.2.3 history describes its Retail display fixes")
    test.eq(info.history[9].changelog[1]:find("跨客户端拍卖编号", 1, true) ~= nil, true,
