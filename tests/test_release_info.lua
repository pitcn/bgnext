return function(test)
    ...
    test.eq(info.version, "0.8.2", "BGNext version is independent")
    ...
    for _, text in ipairs({ "自动拍卖", "买家", "成交金额", "空白装备行", "不覆盖", "权限变化", "欠款" }) do
    ...
    local expectedHistory = { "0.8.1", "0.8.0", "0.7.0", "0.6.0", "0.5.0", "0.4.0", "0.3.1", "0.3.0", "0.2.3", "0.2.2", "0.2.1", "0.2.0", "0.1.0" }
    ...
    test.eq(info.history[9].changelog[1]:find("正式服心愿", 1, true) ~= nil, true,
        "0.2.3 history describes its Retail display fixes")
    test.eq(info.history[10].changelog[1]:find("跨客户端拍卖编号", 1, true) ~= nil, true,
