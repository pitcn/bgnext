return function(test)
    ...
    test.eq(info.version, "0.8.10", "BGNext version is independent")
    ...
    for _, text in ipairs({ "真实交易完成后入账", "默认隐藏非满级角色", "不删除角色数据" }) do
    ...
    local expectedHistory = { "0.8.9", "0.8.8", "0.8.7", "0.8.6", "0.8.5", "0.8.4", "0.8.3", "0.8.2", "0.8.1", "0.8.0", "0.7.0", "0.6.0", "0.5.0", "0.4.0", "0.3.1", "0.3.0", "0.2.3", "0.2.2", "0.2.1", "0.2.0", "0.1.0" }
    ...
    test.eq(info.history[17].changelog[1]:find("正式服心愿", 1, true) ~= nil, true,
        "0.2.3 history describes its Retail display fixes")
    test.eq(info.history[18].changelog[1]:find("跨客户端拍卖编号", 1, true) ~= nil, true,
        "0.2.2 history describes its auction compatibility fixes")
    test.eq(changelogItems:find("Shift+点击", 1, true), nil,
    ...
    test.eq(aboutText:find("BGLite 2.4.2", 1, true) ~= nil, true, "upstream foundation disclosed")
    ...
    test.eq(toc:find("## X-Upstream: BGLite 2.4.2", 1, true) ~= nil, true, "official 2.4.2 baseline metadata")
